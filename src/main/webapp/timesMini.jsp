<%@page import="java.util.Iterator"%><%@page import="java.time.ZoneOffset"%><%@page import="java.util.ArrayList"%><%@page
	import="java.util.List"%><%@page import="java.io.IOException"%><%@page import="praytimes.PrayEvent"%><%@page
	import="java.time.ZoneId"%><%@page import="java.time.OffsetDateTime"%><%@page import="praytimes.Config"%><%@page
	language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%
	response.setHeader("Cache-Control", "public, max-age=9999");
...
%>
<!-- 
	Islamic Prayer Times for <%=cfg.getLocation()%> at <%=today.format(PrayEvent.DATE)%> <%=today.format(PrayEvent.TIME).toLowerCase()%>
	http://<%=cfg.getHostName()%><%=cfg.getPath()%>
-->
...
<%
	for (Iterator<PrayEvent> i = prayers.iterator(); i.hasNext();) {
		...
		String name = "N/A";
		String time = "&ndash;";
		if (prayer.getType() != PrayEvent.Type.InvalidEvent) {
			name = prayer.getType().name().substring(0, 3);
			time = prayer.getFormattedTime().replaceAll(" ([ap])m", "$1");
		}
		if (prayer.getType() == PrayEvent.Type.Suhoor || prayer.getType() == PrayEvent.Type.EndingEvent
				 || prayer.getType() == PrayEvent.Type.Sunrise || prayer.getType() == PrayEvent.Type.Sunset)
			continue;
%>
	<button type="button" class="btn btn-outline-dark" title="<%=prayer.getFormattedDate()%> <%=name%>">
		<small><%=name%><br><%=time%></small>
	</button>
<%
	}
%>
</div>
<div><small><%=today.format(PrayEvent.DATE)%></small></div>
