...
public class PrayTimeFactory {
	 ...
	 * 
	 * fa : fajr angle ms : maghrib selector (0 = angle; 1 = minutes after sunset) mv : maghrib parameter value (in angle or
	 * minutes) is : isha selector (0 = angle; 1 = minutes after maghrib) iv : isha parameter value (in angle or minutes)
	 */
	// private double[] prayerTimesCurrent;
	private int[] offsets;

	public PrayTimeFactory() {
		// Initialize vars
		...
		this.setPreferEnd(4); // Preferred End Time
	...
	public static void main(String[] args) {
		...
		PrayTimeFactory prayers = new PrayTimeFactory();
	...
	private void setThirtyMinutes(int thirtyMinutes) {
		ThirtyMinutes = thirtyMinutes;
	}

	private void setFourtyMinutes(int fourtyMinutes) {
		FourtyMinutes = fourtyMinutes;
	}

	private void setPreferEnd(int preferEnd) {
		PreferEnd = preferEnd;
	}
	...
	public String toString() {
		double[] calcParams = methodParams.get(getCalcMethod());
		Map<String, Object> m = new TreeMap<String, Object>();
		m.put("s", getCalcMethod());
		m.put("fv", calcParams[0]);
		m.put("m", calcParams[1]);
		m.put("mv", calcParams[2]);
		m.put("i", calcParams[3]);
		m.put("iv", calcParams[4]);
		m.put("j", getAsrJuristic());
		m.put("xm", getAdjustHighLats());
		m.put("fo", offsets[0]);
		m.put("do", offsets[2]);
		m.put("ao", offsets[3]);
		m.put("mo", offsets[5]);
		...
		for (Map.Entry<String, Object> pair : m.entrySet()) {
			if (n++ > 0)
				configString.append(':');
			try {
				configString.append(pair.getKey()).append('=').append(URLEncoder.encode(pair.getValue().toString(), "UTF-8"));
